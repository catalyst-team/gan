...
from catalyst_gan.core.state import GanState
from catalyst_gan.core.experiment import GanExperiment
...
class GanRunner(MultiPhaseRunner):
    ...
    def _prepare_for_stage(self, stage: str):
        ...
        if isinstance(model, (dict, nn.ModuleDict)):
            model = nn.ModuleDict({key: nn.DataParallel(value) for key, value in model.items()})
        self.generator = model[self.generator_key]
        self.discriminator = model[self.discriminator_key]
